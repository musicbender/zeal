...
sudo chown pi:pi /etc/gaspar /etc/worfbot-gateway
...
cat <<'EOF' | sudo tee /etc/sudoers.d/pi-apps > /dev/null
pi ALL=(ALL) NOPASSWD: /bin/systemctl restart gaspar, /bin/systemctl restart worfbot-gateway
EOF
sudo chmod 440 /etc/sudoers.d/pi-apps
