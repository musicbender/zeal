...
const log = initLogger('sensors.routes');

export async function registerSensorRoutes(server: FastifyInstance, sensorService: SensorService) {
	server.post<{ Body: CreateSensorDto }>('/sensors', async (request, reply) => {
		try {
			...
			log.error({ error }, 'Failed to create sensor');
	...
	server.patch<{ Params: { id: string }; Body: UpdateSensorDto }>(
		...
		async (request, reply) => {
			try {
				...
				log.error({ error, id: request.params.id }, 'Failed to update sensor');
				return reply.status(400).send({ error: 'Failed to update sensor' });
			}
		}
	);

	server.delete<{ Params: { id: string } }>('/sensors/:id', async (request, reply) => {
		try {
			const { id } = request.params;
			await sensorService.remove(id);
			return reply.status(204).send();
		} catch (error) {
			log.error({ error, id: request.params.id }, 'Failed to delete sensor');
