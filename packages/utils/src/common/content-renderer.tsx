...
export function renderRichTextNode(node: RichTextNode, i: number): React.ReactNode {
	...
	if (node.type === 'bulleted-list') {
		return <ul key={i}>{node.children?.map((child, j) => renderRichTextNode(child, j))}</ul>;
	}

	if (node.type === 'numbered-list') {
		return <ol key={i}>{node.children?.map((child, j) => renderRichTextNode(child, j))}</ol>;
	}

	if (node.type === 'list-item') {
		return <li key={i}>{node.children?.map((child, j) => renderRichTextNode(child, j))}</li>;
	}
